      ...
      <div className="mt-6">
        ...
        <div className="w-full h-8 mt-2">
          ...
          <div className="flex flex-row justify-end gap-4 mt-2">
            ...
            {comment.length>0?(  <div className="font-prompt w-auto rounded-full p-2 h-10 bg-blue-700 text-white">
              <button onClick={handleSubmit}>Comment</button>
            </div>):(  <div className="font-prompt w-auto rounded-full p-2 h-10 ">
              <button onClick={handleSubmit}>Comment</button>
            </div>)}
      ...
      <div>
        {showComment.map((comment, index) => (
          <div key={index} className="mt-8">
            ...
            <p className="pl-5"> {new Date(comment.Date).toLocaleString()}</p>{" "}

            </div>
    
            <p className="pl-14 pb-4 font-prompt">{comment.comment}</p> {/* Render the comment text */}
            {/* Render the date */}
          </div>
        ))}
      </div>
